...
curd = getwd()
setwd(td)
if (!file.exists("core_src.zip"))
   system(paste0("cp ", system.file("core_src.zip", package="BiocBuildTools"), " ", td), intern=TRUE)
if (!dir.exists("core_src")) unzip("core_src.zip")
...
```{r doch, cache=FALSE}
setwd(td)
chk_parody = rcmdcheck::rcmdcheck("core_git/parody", quiet=TRUE)
names(chk_parody)
setwd(curd) # for knitr
```
